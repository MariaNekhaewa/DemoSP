...
namespace ПМ
{
    public partial class ClientForm : Form
    {
        ...
        private void LoadData()
        {
            ...
            dataGridViewClients.Columns["client_info_id"].Visible = false;
            dataGridViewClients.Columns["client_fio"].HeaderText = "ФИО клиента";
            dataGridViewClients.Columns["client_telephone_number"].HeaderText = "Телефон клиента";
            dataGridViewClients.Columns["problem"].HeaderText = "Проблема";
            dataGridViewClients.Columns["date_of_the_end"].HeaderText = "Дата окончания";
            dataGridViewClients.Columns["phone_name"].HeaderText = "Название телефона";
            dataGridViewClients.Columns["cost"].HeaderText = "Стоимость";
        ...
        private void toolStripTextBox1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                int clientId = (int)dataGridViewClients.CurrentRow.Cells["client_info_id"].Value;

                NpgsqlCommand cmd = new NpgsqlCommand($"UPDATE client_info SET {dataGridViewClients.CurrentCell.OwningColumn.HeaderText} = '{toolStripTextBox1.Text}'" +
        ...
        private void dataGridView1_CellMouseDown(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            {
                dataGridViewClients.CurrentCell = dataGridViewClients.Rows[e.RowIndex].Cells[e.ColumnIndex];
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            string client_fio = dataGridViewClients.Rows[rowIndex].Cells["client_fio"].Value.ToString();
            string client_telephone_number = dataGridViewClients.Rows[rowIndex].Cells["client_telephone_number"].Value.ToString();
            string problem = dataGridViewClients.Rows[rowIndex].Cells["problem"].Value.ToString();
            string date_of_the_end = dataGridViewClients.Rows[rowIndex].Cells["date_of_the_end"].Value.ToString();
            string phone_name = dataGridViewClients.Rows[rowIndex].Cells["phone_name"].Value.ToString();
            string cost = dataGridViewClients.Rows[rowIndex].Cells["cost"].Value.ToString();
